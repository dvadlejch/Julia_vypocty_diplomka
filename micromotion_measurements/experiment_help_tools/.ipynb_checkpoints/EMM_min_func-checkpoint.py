...
def get_hist_fit(fotkor, t_res, t_measure, background_photocounts, hist_sigma, max_phi_unc = 0.3, phi0 = 1.1, 
                 voltages=np.array([[1,1], [1,1]]), sign_DeltaS = False):
...
## defining a function calculating the fluorescence signal for given parameters - single signal

def get_hist_fit_single(fotkor_var, t_res, t_measure, background_photocounts, hist_sigma, max_phi_unc = 0.3, phi0 = 1.1, 
                 voltages=np.array([[1,1], [1,1]])):
    # function returns DeltaS_S_ratio, Delta_S_S_ratio_sigma, fot_phi, fot_phi_sigma, hist_sigma

    nu = (voltages[:,0] - voltages[:,1]) / (voltages[:,0] + voltages[:,1])
    # pomocne promene
    fotkor_shape = np.shape(fotkor_var)
    # casova skala foton-kor. dat
    t_scale = np.array(range(0, fotkor_shape[0])) * t_res

    # ----- odecet pozadi
    bg_ph_sum = background_photocounts * t_measure  # celkovy pocet fotonu pozadi za cas mereni
    last_bin_ratio = fotkor_var[fotkor_shape[0] - 2] / fotkor_var[fotkor_shape[0] - 3]  # pomer mezi county v poslednim/predposlednim binu
    bg_ph_per_bin = bg_ph_sum / (fotkor_shape[0] - 2 + last_bin_ratio)

#     fotkor_var = fotkor[:fotkor_shape[0] - 2] - bg_ph_per_bin
#     fotkor_var = np.append(fotkor_var, fotkor[fotkor_shape[0] - 2] - bg_ph_per_bin * last_bin_ratio)
#     fotkor_var = np.append(fotkor_var, 0)
    fotkor_var[:fotkor_shape[0] - 2] = fotkor_var[:fotkor_shape[0] - 2] - bg_ph_per_bin
    fotkor_var[fotkor_shape[0] - 2] = fotkor_var[fotkor_shape[0] - 2] - bg_ph_per_bin * last_bin_ratio
    #-----------------------------

    #------- odhad RF frekvence i s nejistotou
    # odhad periody triggeru
    T_trig = (fotkor_var[fotkor_shape[0] - 2] / fotkor_var[fotkor_shape[0] - 3]) * t_res + t_scale[fotkor_shape[0] - 2]
...
#     fot_phi_sigma = []
        ...
        # --- zde budu zkouset postupne ruzne pocatecni body tak, aby minimalizace vybrala globalni minimum
    x0 = [fotkor_var[:fotkor_shape[0] - 2].mean(), 0.5 * (fotkor_var[:fotkor_shape[0] - 2].max() - fotkor_var[:fotkor_shape[0] - 2].min()) /
          fotkor_var[:fotkor_shape[0] - 2].mean(), phi0]
        #     x0 = [fotkor[:fotkor_shape[0]-2,i].mean(), 0.5* ( fotkor[:fotkor_shape[0]-2,i].max()
        #         -fotkor[:fotkor_shape[0]-2,i].min() )/fotkor[:fotkor_shape[0]-2,i].mean(), phi0, 100]
        #     fit = minimize(likehood_transform, x0, args=(Omega, fotkor[:fotkor_shape[0]-2,i], t_res, hist_sigma[i] ), tol=1e-10 )
    fit = minimize(likehood_transform, x0, args=(Omega, fotkor_var[:fotkor_shape[0] - 2], t_res, hist_sigma),tol=1e-10,
                   jac=likehood_transform_jac)
        #     fit = minimize(likehood_transform_sigma, x0, args=(Omega, fotkor[:fotkor_shape[0]-2,i], t_res), tol=1e-10)
        #     print(fit)
        #     print('\n')
...
#             pass
    ...
    return( DeltaS_S_ratio, Delta_S_S_ratio_sigma, fot_phi, fot_phi_sigma, x, Omega, t_scale )